...
public:
    ...
    constexpr bool is_valid_index(const u32 idx) const
    {
        return idx < N;
    }
...
public:
    ...
    explicit Array(IAllocator& allocator, u32 reservedNum = 4) : _Allocator(allocator)
    {
        Data = _Allocator.CreateArray<T>(reservedNum);
        _NumAllocated = reservedNum;
    }

    explicit Array(IAllocator& allocator, std::initializer_list<T> initList) : _Allocator(allocator)
    {
    ...
    void Resize(u32 newSize)
    {
        ...
        assert(temp != nullptr);
        //copy over old data to new TODO: what if we work with a paged allocator?
...
template<typename T, u32 N>
inline View<T> CreateView(StaticArray<T, N>& array, u32 startIndex = 0, u32 size = N)
{
...
template<typename T, u32 N>
inline View<const T> CreateView(const StaticArray<T, N>& array, u32 startIndex = 0, u32 size = N)
{
    const u32 size_clamped = std::min(N - startIndex, size);
    View<const T> Result = {
        .Data = &array[startIndex],
